...
class FDP_WorkFlow(object):
    ...
    def _load_param(self, param):
        return ParamExtract.recursive_parse_param_from_config(param, self.config_json, self.valid_classes,
                                                              param_parse_depth=0)

    def _initialize_workflow_param(self):
        workflow_param = WorkFlowParam()
        self.workflow_param = self._load_param(workflow_param)
    ...
    def feature_binning(self, data_instances, flow_id='sample_flowid'):
        ...
        feature_binning_param = self._load_param(feature_binning_param)
    ...
    def feature_selection_fit(self, data_instance, flow_id='sample_flowid', without_transform=False):
        ...
        if self.workflow_param.need_feature_selection:
            ...
            feature_select_param = self._load_param(feature_select_param)
    ...
    def sample(self, data_instance, sample_flowid="sample_flowid"):
        ...
        sample_param = self._load_param(sample_param)
    ...
    def cross_validation(self, data_instance):
        ...
        if self.role == consts.GUEST or (self.role == consts.HOST and self.mode == consts.HOMO):
            ...
            self.cv_result = format_cv_result
    ...
    def save_predict_result(self, predict_result):
        # print('save predict')
        # print(self.workflow_param.predict_output_table, self.workflow_param.predict_output_namespace)
    ...
    def save_intersect_result(self, intersect_result):
        if intersect_result:
            ...
            LOGGER.info("Save intersect results DONE")
        else:
    ...
    def scale(self, data_instance, fit_config=None):
        if self.workflow_param.need_scale:
            ...
            self.scale_params = self._load_param(scale_params)
    ...
    def gen_data_instance(self, table, namespace, mode="fit"):
        ...
        if mode == "fit":
            ...
            for meta_buffer_type, param_buffer_type in save_result:
                self.pipeline.node_meta.append(meta_buffer_type)
                self.pipeline.node_param.append(param_buffer_type)
        LOGGER.debug("mode is {} done".format(mode))
        return data_instance
    ...
    def _init_argument(self, config_json, job_id):
        ...
        self.valid_classes = [class_info[0] for class_info in inspect.getmembers(param, inspect.isclass)]
